...
class AttackResolution:
    def __init__(self, hit=False, damage=0, roundtime=0, messages=None, details=None):
        self.hit = hit
        self.damage = damage
...
def calculate_hit(attacker, target, context=None):
    ...
    # MOVE_TO_DOMAIN: hit calculation
...
def calculate_damage(attacker, target, context=None):
    ...
    # MOVE_TO_DOMAIN: damage calculation
    profile = context["profile"]
    ...
    if snipe_active and hasattr(attacker, "get_wilderness_bond") and hasattr(attacker, "get_nature_focus"):
        if attacker.get_wilderness_bond() >= int(RANGER_SNIPE_CONFIG.get("mastery_bond_threshold", 80) or 80) and attacker.get_nature_focus() >= int(RANGER_SNIPE_CONFIG.get("mastery_focus_threshold", 60) or 60):
