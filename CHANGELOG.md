...
- Normalized canonical Landing visibility across runtime and builder surfaces. `world/area_forge/map_api.py` now prefers room-owned zone metadata instead of falling back to `local`/raw area tags for canonical rooms, and startup in `server/conf/at_server_startstop.py` now boots through `ensure_full_canonical_crossing()` instead of the old phase chain, skips the procedural `build_the_landing(...)` startup build, primes `the_landing` cache state, and self-heals canonical arrival by rebuilding the full canonical graph if the required guildhall stub destinations are no longer reachable from canonical arrival.
- Completed direct canonical-arrival migration and procedural `new_landing` retirement. `systems/canonical_migration.py` now classifies procedural rooms by `build:new_landing` instead of broad area-name matching, retries lock-prone SQLite writes/deletes, tolerates stale Evennia objects during cleanup, and falls back to direct persisted relocation when session-backed `move_to()` is unsafe. Live cleanup completed successfully: remaining procedural Landing rooms dropped to `0`, and the last stranded characters were migrated to canonical arrival room `48827`.
- Preserved the historical phase functions as deprecated-but-functional entry points while making them safe against the now-live full canonical graph. `world/areas/the_crossing/import_canonical.py` now resynchronizes canonical-managed exits for touched rooms before phase imports add the current-map exits, de-duplicates stale canonical exits, and applies lock-aware / already-deleted-safe deletion semantics so the phase tests remain deterministic without broadening into fixture or startup rewrites.
- Added `tests/world/test_canonical_crossing_full_import.py` with focused coverage for the consolidated importer, Landing metadata, builder/map visibility, fixture safety, procedural cleanup, and tutorial-threshold routing. The new file landed at `10 passed` and stayed green after the importer and migration hardening work.
- The canonical Crossing rebuild program is now complete. Core Crossing import, arrival, zone identity, stub reachability, and procedural Landing retirement are closed; remaining work is frontier/adjacent-area/profession expansion, not more core canonical Crossing rebuild work.

Validation:
- New consolidated regression file: `10 passed` across `tests/world/test_canonical_crossing_full_import.py`.
- Broader Crossing/Landing slice: `63 passed` across canonical phase 1-6, guildhall stubs, canonical arrival path, Landing learning coverage, and fixture-safety protection.
...
- Live runtime probe after cleanup confirmed `room_count = 253`, canonical arrival `48827 / Town Green North` tagged as `zone = The Landing`, builder discovery sees `the_landing` with `253` rooms, map payload reports `zone = The Landing`, and `remaining_procedural_rooms = 0`.

### DRG-CANONICAL-FIXTURE-SAFETY-001

- Shipped the canonical test-fixture cascade block before any substrate re-materialization. `tools/diretest/core/harness.py` now refuses to delete canonical-tagged room objects during `safe_delete(...)`, logs the protected object id plus canonical metadata plus calling test context, and stops cascade descent at that room boundary. `tests/fixtures/safe_smoke_delete.py` now adds a second guard layer in front of `safe_delete(...)`, filtering canonical-tagged rooms before delegation and reporting `deleted_count` / `filtered_count` while preserving the existing failure-list API.
- Repaired all affected canonical phase/stub tests so importer output is no longer treated as teardown-owned fixture state. `tests/world/test_canonical_crossing_phase1.py`, `test_canonical_crossing_phase2.py`, `test_canonical_crossing_phase3.py`, `test_canonical_crossing_phase4.py`, `test_canonical_crossing_phase5.py`, `test_canonical_crossing_phase6.py`, and `test_canonical_guildhall_stubs.py` no longer append `ensure_*()` results into `self.created`. `tests/world/test_canonical_arrival_path.py` was intentionally left untouched because RECOVERY-AUDIT-001 already proved it safe.
- Added the regression file `tests/fixtures/test_safe_delete_canonical_protection.py`, including the would-have-caught-it importer regression: `ensure_canonical_crossing_phase1()` output survives `safe_smoke_delete()` with identical before/after ids in the current degraded live DB. The wrapper regression now also proves caller-awareness counts: canonical rooms are filtered, regular rooms are deleted, and the returned object exposes `filtered_count` / `deleted_count` without breaking existing `failures == []` expectations.
- Actual scope stayed inside the dispatch ceiling. Production changes were `70` added / `1` removed across `tools/diretest/core/harness.py` (`54` added) and `tests/fixtures/safe_smoke_delete.py` (`16` added / `1` removed). Fix C changed `24` added / `79` removed across the seven canonical phase/stub test files. Fix D added `100` lines in the new regression file.
- Retrospective discipline correction captured by this dispatch: the repeated REBUILD-002 through REBUILD-006 classification of canonical-map-id assertion failures as "environment-sensitive" was wrong. On canonical-tagged rooms, that pattern now defaults to root-cause investigation, not deferred-defect classification.
- The fixture-safety fix does not re-materialize the missing substrate. Live importer-output smoke proves protection works in the current degraded state: Phase 1 room ids before `safe_smoke_delete()` were `[48827, 48828]`, after re-running `ensure_canonical_crossing_phase1()` they remained `[48827, 48828]`, with `filtered_count = 2`, `deleted_count = 0`, and no failures. The live canonical room count remains `155`, so the next dispatch is still `DRG-CANONICAL-SUBSTRATE-REMATERIALIZE-001` before FIX-001 validation resumes.

Validation:
- Fixture-safety regression + repaired canonical phase slice: `21 passed`.
- Remaining touched canonical phase/stub files: `17 passed`.
- Exact documented preservation batch reran clean at `315 passed, 153 subtests passed`.
- Exact documented Ranger-adjacent batch reran clean at `92 passed, 138 subtests passed`.
- Untouched `tests/world/test_canonical_arrival_path.py` still has one live assertion failure in `test_audit_stub_destinations_are_reachable_from_canonical_arrival`; per dispatch constraints that file was not edited, and the failure remains substrate-blocked pending `DRG-CANONICAL-SUBSTRATE-REMATERIALIZE-001`.

### DRG-CANONICAL-SUBSTRATE-RECOVERY-AUDIT-001

- Closed as **Outcome C**: canonical substrate loss was caused by test-fixture cascade deletion, not by the committed FIX-001 bootstrap edits and not by evidence of a DB restore. Live read-only inventory found the canonical graph badly degraded rather than wholly absent: phase counts had fallen to `25 / 25 / 24 / 60 / 14 / 3 / 4` for phases `1-6` plus `guildhall_stub`, with only `155` rooms still carrying any `canonical_map_id`, zero live `Town Green North` key matches, and audited room id `47727` no longer present in the database.
- Restart history ruled out today's `at_server_startstop.py` changes as the immediate cause. `git log -p server/conf/at_server_startstop.py` shows the canonical bootstrap chain was added in commit `272002d` at `2026-05-18 13:20`, but `portal.log` shows the most recent actual server startup was `2026-05-17 10:01:30` and the process remained alive through today via continuous timeout activity. The current bootstrap ordering is canonical-after-procedural and therefore not the active cause of the live disappearance observed during FIX-001.
- Backup inspection also did not support a restore explanation. The newest backup artifacts remain yesterday's `server/evennia.db3.backup-V2-20260517-092140` and `server/evennia.db3.backup-20260517-084034`, while the live `server/evennia.db3` was modified today at `2026-05-18 13:23:49` and is larger than either backup. No newer `*.db3` backup or restore artifact was found.
- The decisive evidence came from the touched canonical test suite. `tests/world/test_canonical_arrival_path.py` itself is safe because it only deletes rooms explicitly created in the test. But the newly added canonical phase and guildhall test files call `ensure_canonical_crossing_phase1()` through `phase6()` and `ensure_canonical_guildhall_stubs()`, append those returned persistent rooms into `self.created`, and then call `safe_smoke_delete(*self.created)` in teardown. `tests/fixtures/safe_smoke_delete.py` delegates to `tools/diretest/core/harness.py::safe_delete(...)`, which recursively walks outgoing exits, incoming exits, and nested contents and then calls `obj.delete()` on each persisted object. The current live missing-id pattern aligns directly with the ids exercised by those tests, including `788`, `768`, `769`, `771`, `821`, `825`, `899`, `915`, `735`, `742`, `7898`, `958`, `7900`, `5995`, and `9077`.
- Code-level importer substrate remains intact. Read-only probes confirmed `data/canon/map-1777858104.json` still exists and parses, `extract_crossing_entries()` still finds `238` Crossing entries, the expected room-id constants remain `30 / 32 / 33 / 71 / 35 / 33` plus `19` guildhall stubs, and `phase1_entries_from_map()` through `phase6_entries_from_map()` plus `guildhall_stub_entries_from_map()` all resolve at the expected counts. Recovery is therefore a re-materialization plus fixture-safety problem, not an importer-logic repair problem.
- Sized recovery recommendation: follow with `DRG-CANONICAL-FIXTURE-SAFETY-001`, scoped to hardening the canonical test teardown path so persisted `ensure_*()` results are never handed to recursive deleters, then re-materialize the canonical Crossing and guildhall-stub substrate from the intact map importer, then resume FIX-001 validation. Estimated production scope: `30-100` lines across the fixture harness and touched canonical tests, plus a small canonical re-materialization dispatch after the safety repair. FIX-001's committed routing code can likely be re-validated as-is once substrate is restored and the destructive fixture path is blocked.

Validation:
- Read-only diagnostic dispatch only: no production code, test code, or live DB writes were performed during the audit.
- Live probes confirmed: `get_canonical_crossing_arrival_room() -> None`, `_resolve_landing_arrival_room() -> Bellfound Steps (#4291)`, `Town Green North` key matches = `0`, audited object id `47727` missing.
- Locked anchors remain unchanged from last verified state: preservation `315 passed, 153 subtests passed`; Ranger-adjacent `92 passed, 138 subtests passed`.
