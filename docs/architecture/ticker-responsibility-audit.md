...
| Cyclic magic sustain | `process_status_tick()` -> `process_cyclic()` | `KEEP` | Medium. Runs after general magic-state upkeep because active cyclic state depends on current magic flags. | High. Depends on active cyclic spell state and resource pools. | Cyclic effects are recurring upkeep, not a one-shot boundary. Keep on the shared ticker for now. |
